...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case S2C_MOVE_OBJECT: {
		...
		int obj_id = move_packet->object_id;

		if (obj_id < MAX_CLIENTS && client_map[obj_id] != -1) {
			int my_id = client_map[obj_id];
...
void Test_Thread()
{
	while (true) {
		...
		for (int i = 0; i < num_connections; ++i) {
			...
			if (g_clients[i].last_move_time + 500ms > now) continue;
			g_clients[i].last_move_time = now;

			if (g_clients[i].x >= 900 && g_clients[i].x <= 1100 &&
				g_clients[i].y >= 900 && g_clients[i].y <= 1100) {
			...
			C2S_Move mv_packet;
			mv_packet.size = sizeof(mv_packet);
			mv_packet.type = C2S_MOVE;
			...
			mv_packet.x = dx;
			mv_packet.y = dy;
			mv_packet.move_time = static_cast<unsigned>(duration_cast<milliseconds>(now.time_since_epoch()).count());
			SendPacket(i, &mv_packet);
		}
...
void InitializeNetwork()
{
	...
	wcout << L"서버 IP 주소를 입력하세요 (기본값: 127.0.0.1): ";
